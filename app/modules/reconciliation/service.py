...
def _text_score(a: str | None, b: str | None) -> tuple[float, list[str]]:
    ...
    if a in b or b in a:
        ...
        return 15.0, reasons
    # lightweight overlap
    aset = set([t for t in a.split() if len(t) > 3])
    bset = set([t for t in b.split() if len(t) > 3])
...
def score_match(invoice: Invoice, tx: BankTransaction, window_days: int = 3) -> Candidate | None:
    ...
    return Candidate(invoice_id=invoice.id, bank_transaction_id=tx.id, score=round(score, 3), reasons=reasons)

class ReconciliationService:
    def __init__(self, session: Session):
        self.session = session

    def reconcile(self, tenant_id: int, window_days: int = 3, max_candidates_per_invoice: int = 3) -> list[Match]:
        # Clear previous proposed matches for a deterministic MVP behavior
        self.session.execute(delete(Match).where(Match.tenant_id == tenant_id, Match.status == "proposed"))
        self.session.commit()

        invoices = list(self.session.scalars(
            select(Invoice).where(Invoice.tenant_id == tenant_id, Invoice.status == "open").order_by(Invoice.id.asc())
        ).all())
        txs = list(self.session.scalars(
            select(BankTransaction).where(BankTransaction.tenant_id == tenant_id).order_by(BankTransaction.id.asc())
        ).all())
    ...
    def build_explain_context(self, tenant_id: int, invoice_id: int, transaction_id: int, window_days: int = 3) -> ExplainContext:
        ...
        tx = self.session.scalars(
            select(BankTransaction).where(BankTransaction.tenant_id == tenant_id, BankTransaction.id == transaction_id)
        ).first()
        if not tx:
