...
class BankTransactionService:
    ...
    def import_bulk(self, tenant_id: int, idempotency_key: str, items: list[dict]) -> dict:
        ...
        # Basic dedupe: by external_id if provided, else by (posted_at, amount, description)
        imported = 0
        ...
        result = {"imported": imported, "deduped": duplicate_external_ids, "duplicate_external_ids": duplicate_external_ids, "transaction_ids": transaction_ids}
        idem = IdempotencyKey(
            tenant_id=tenant_id,
            key=idempotency_key,
